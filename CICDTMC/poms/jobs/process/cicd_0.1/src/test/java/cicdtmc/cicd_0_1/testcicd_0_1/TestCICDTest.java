...
package cicdtmc.cicd_0_1.testcicd_0_1;
...
/**
 * Job: TestCICDTest Purpose: <br>
 * Description: <br>
 * 
 * @author MOUSSAOUI, Massine
 * @version 7.3.1.20210623_0656-patch
 * @status
 */
public class TestCICDTest implements TalendJob {
	static {
		System.setProperty("TalendJob.log", "TestCICDTest.log");
	}

	private static org.apache.logging.log4j.Logger log = org.apache.logging.log4j.LogManager
			.getLogger(TestCICDTest.class);
	...
	private final String jobName = "TestCICD";
	private final String projectName = "CICDTMC";
				...
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(TestCICDTest.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
	...
	public static class ContextBean {
		static String evaluate(String context, String contextExpression)
			...
			java.io.InputStream inContext = TestCICDTest.class.getClassLoader()
					.getResourceAsStream("cicdtmc/cicd_0_1/testcicd_0_1/contexts/" + context + ".properties");
			if (inContext == null) {
				inContext = TestCICDTest.class.getClassLoader()
	...
	public static class row2Struct implements routines.system.IPersistableRow<row2Struct> {
		final static byte[] commonByteArrayLock_CICDTMC_TestCICD = new byte[0];
		static byte[] commonByteArray_CICDTMC_TestCICD = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_CICDTMC_TestCICD.length) {
					if (length < 1024 && commonByteArray_CICDTMC_TestCICD.length == 0) {
						commonByteArray_CICDTMC_TestCICD = new byte[1024];
					} else {
						commonByteArray_CICDTMC_TestCICD = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_CICDTMC_TestCICD, 0, length);
				strReturn = new String(commonByteArray_CICDTMC_TestCICD, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_CICDTMC_TestCICD) {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				for (int i_tFixedFlowInput_1 = 0; i_tFixedFlowInput_1 < 10; i_tFixedFlowInput_1++) {
					...
					if (row2.nom != null) {
						sb_tFileOutputDelimited_1.append(row2.nom);
					}
				...
				globalMap.put("tFixedFlowInput_1_NB_LINE", 10);
	...
	public static void main(String[] args) {
		final TestCICDTest TestCICDTestClass = new TestCICDTest();

		int exitCode = TestCICDTestClass.runJobInTOS(args);
		if (exitCode == 0) {
			log.info("TalendJob: 'TestCICD' - Done.");
		}
	...
	public void testDefault() throws java.lang.Exception {
		...
		TestCICDTestClass.runJobInTOS(arrays);
		...
		if (TestCICDTestClass.exception != null) {
			throw TestCICDTestClass.exception;
		}
	}
	...
	public int runJobInTOS(String[] args) {
		...
		log.info("TalendJob: 'TestCICD' - Start.");
		...
		try {
			...
			java.io.InputStream inContext = TestCICDTest.class.getClassLoader()
					.getResourceAsStream("cicdtmc/cicd_0_1/testcicd_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = TestCICDTest.class.getClassLoader()
		...
		if (false) {
			System.out
					.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : TestCICDTest");
		}
		if (enableLogStash) {
...
/************************************************************************************************
 * 77738 characters generated by Talend Cloud Real-Time Big Data Platform on the
 * June 16, 2022 3:35:16 PM CEST
 ************************************************************************************************/